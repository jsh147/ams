...
public class SoldierDAO {
	...
	public boolean addSoldier(SoldierDTO newSoldier) {
		try {
			String sql = "insert into soldier (Sarmycode, Sgrade, Sname, Sbirth, Sstate, Sgunnum, Scamp) values(?,?,?,?,?,?,?)";
			...
			ps.setInt(6, newSoldier.sgunnum);
	...
	public boolean modifyGrade(String Sgrade, String Sarmycode) {
		try {
			String sql = "update soldier set Sgrade = ? where Sarmycode = " + Sarmycode;
			ps = conn.prepareStatement(sql);
		 ...
		 try {
			...
			ps.setString(1, milinum);
			 ...
			 while(rs.next()) {
				 ...
				 Soldierdto.sgunnum = rs.getInt("sgunnum");
