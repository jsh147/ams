...
public class ManagerDao {
	...
	public boolean join(ManagerDTO newManager) {
		try {
			String sql = "insert into Muser(Marmycode, Mpassword, Mgrade, Mjob, Mname, Mbirth, Mgunnum, Mcamp) values(?,?,?,?,?,?,?,?)";
			ps = conn.prepareStatement(sql);
	...
	public boolean login(String Mamrycode, String Mpassword) {
		try {
			...
			if(rs.next()) {
				...
				loginManager.Mcamp = rs.getInt("Mcamp");
	...
	public boolean leaveID(String Marmycode) {
		try {
			String sql = "delete from user where Marmycode = ?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, Marmycode);
	...
	public boolean modifyBelong(String Nbelong, String Marmycode) {
		// 사단변경
		try {
			String sql = "update Muser set Mcamp = ? where Marmycode = " + Marmycode;
			ps = conn.prepareStatement(sql);
			ps.setString(1, Nbelong);
			return ps.executeUpdate() == 1;
		} catch (SQLException e) {
		}
		return false;
